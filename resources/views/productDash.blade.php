...
<body class="flex flex-col md:flex-row h-auto min-h-screen bg-gray-50">
    <!-- bar vertical -->
                            ...
                            <tr class="border-t">
                                ...
                                <td class="px-6 py-3 w-48">
                                    <a href="{{ route('posts.show', $post) }}">
                                        <img class="w-full object-cover" src="{{ $post->image }}" alt="post picture">
                                    </a>
                                </td>
                                ...
                                <!-- <td class="px-6 py-3 text-xl"> {{ $post->description }}</td> -->
                                <td class="px-6 py-3 "><a class="text-blue-500"
                                        href="{{ $post->url }}">{{ substr($post->url, 0, 15) }}...</a></td>

                                <td class="px-6 py-3  "> {{ $post->comments->count() }}</td>
                                <td class="px-6 py-3 "> {{ $post->likedByUsers->count() }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
